...
<div>
    ...
    <div class="flex px-4 py-2 justify-between">
        <h1 class="text-3xl font-medium text-gray-700 dark:text-white">All Sub Categories</h1>
        @if(!empty($PermissionAdd))
        <button type="button" onclick="window.location='{{ route('sub-category.create') }}'" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Add SubCategory</button>
        @endif
    </div>
                    ...
                    <tbody class="">
                        <tr>
                            ...
                            <td class="px-6 py-2 text-center whitespace-no-wrap border-b border-gray-200">
                                <form action="{{ route('sub-category.destroy', $subCategory->id) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
